...
public MyResult drawscrn()
{
	...
	int nsdivx = 2;
	if (nsamp == 600 && tbms[tbval] == 1.0f)
		nsdivx = 5;
	if (nsamp == 600 && tbms[tbval] == 2.0f)
		nsdivx = 4;
	if (nsamp == 600 && tbms[tbval] == 5.0f)
		nsdivx = 5;
	if (nsamp == 600 && tbms[tbval] == 10.0f)
		nsdivx = 5;
	if (nsamp == 600 && tbms[tbval] == 20.0f)
		nsdivx = 4;
	if (nsamp == 600 && tbms[tbval] == 50.0f)
		nsdivx = 5;
	if (nsamp == 600 && tbms[tbval] == 100.0f)
		nsdivx = 5;
	if (nsamp == 300)
		nsdivx = 10;
...
public void printtraces(String name)
{
	...
	for (int ichan = 0; ichan < maxnchan; ichan++)
	{
		...
		for (int isamp = 0; isamp < nsamp; isamp++)
		{
			if (i == ichan)
			{
				float x = tbms[tbval] * (isamp * (w_scrn / (1.0f * nsamp))) / xdivdist;
				float y = (values[isamp] + 2.0f) * (5.00f / (1.0f * maxval));
				output.println(x + ", " + y);
			}
			i = nextchan[i];
		}
...
public double get_reading(int active_chan) {
...
public double get_resistance (double v_avg){

	return (ref_resistor*v_avg)/(maxval - v_avg);

}
public void drawOhmmeter() {
	...
	fill(0);
	stroke(0);
	rect(0, y_scrn + 0.5f * h_scrn, 1100, 80);

	rect(0, y_scrn + 0.75f * h_scrn, 1100, 80);
	...
	if (resistance_reading < 10 || resistance_reading > 33000){
		...
		result = nf(resistance_reading/1000.0f, 1, 2);
		unit = "kOhm";
	}
	else {
		result = nf(resistance_reading/1000.0f, 2, 1);
		unit = "kOhm";
	}
	text(result, x_scrn + 0.5f * w_scrn, y_scrn + 0.5f * h_scrn + 80);
	textFont(f, 16);
	text(unit, x_scrn + 0.5f * w_scrn + 4*textWidth(result), y_scrn + 0.5f * h_scrn );
}

public void setUpOhmmeter(){
	fill(0);
	stroke(0);
	rect(0,0,1100, 800);
	...
	while (true){
		...
		try{
			ref_resistor = PApplet.parseFloat(r_val);
			...
			if (trig_mode == 0)
			{
				...
				if (mouseX > x_pb && mouseX < x_scrn+w_scrn && mouseY > y_pb && mouseY < y_pb + 4 * h_pb)
				{
					if (mouseY < y_pb + 2 * h_pb)
						pls_ip = constrain(PApplet.parseInt((mouseX - x_pb) / (w_pb / pls_np)), 0, pls_period.length - 1);
					if (mouseY > y_pb + 2 * h_pb)
						pls_il = constrain(PApplet.parseInt((mouseX - x_pb) / (w_pb / pls_nl)), 0, pls_len[0].length - 1);
...
class CursorBars
{
	...
	public void display(MyResult spacings)
	{
...
class XCursorBar
{
	...
	public void move(int which_bar, boolean direction)
	{
		if (which_bar == 1)
		{
			xc1.move(direction);
	...
	//draw the bar for the vertical scale
	public void drawxcb(float x_spacing)
	{
		fill(0);
		stroke(0);
		...
		text(result, x_xcb + 0.5f * w_xcb, y_xcb + 1 * h_tb - 2);
	}
}
...
class HScrollbar
{
	...
	private int cursor_selected_color;

	HScrollbar(float xp, float yp, int sw, int sh, float sp, int l, int cc, int csc)
	{
		swidth = sw;
		sheight = sh;
	...
	public void move(boolean left_or_right)
	{
		if (left_or_right)
		{
			newspos -= 0.5f;
	...
	public boolean get_mousePressed_over()
	{
...
class YCursorBar
{
	...
	//draw the bar for the vertical scale
	public void drawycb(float y_spacing)
	{
		fill(0);
		stroke(0);
		...
		text(result, x_ycb + 0.5f * w_ycb, y_ycb + 1 * h_tb - 2);
	}
}
...
class VScrollbar
{
	...
	VScrollbar(float xp, float yp, int sw, int sh, float sp, int l, int cc, int csc)
	{
		swidth = sw;
		sheight = sh;
		int widthtoheight = sw - sh;
	...
	public void move(boolean up_or_down)
	{
		if (up_or_down)
		{
			newspos -= 0.5f;
